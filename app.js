...
function cargarTerminados() { 
    google.script.run.withSuccessHandler(d => { 
        ...
        if(typeof lucide !== 'undefined') lucide.createIcons(); 
    }).obtenerLogistica({ tipo: 'TERMINADOS' }); // 👈 CAMBIO AQUÍ: Se envía como objeto
}
...
function cargarAlquiler() { 
    google.script.run.withSuccessHandler(d => { 
        ...
        d.forEach((r, i) => { 
            ...
            else if (r.estado === "PRESTADO") badgeClass = 'bg-blue-100 text-blue-700'; 
        ...
        if(typeof lucide !== 'undefined') lucide.createIcons(); 
    }).obtenerLogistica({ tipo: 'ALQUILER' }); // 👈 CAMBIO AQUÍ: Se envía como objeto
}
...
function cargarPatio() { 
    ...
    }).obtenerLogistica({ tipo: 'PATIO' }); // 👈 CAMBIO AQUÍ: Se envía como objeto
}
