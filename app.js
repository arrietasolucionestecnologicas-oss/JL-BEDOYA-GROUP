/* JLB OPERACIONES - APP.JS (V24.6 - FIX SALTO ESTADO + FOTOS INTACTAS) */
...
function insertarFilaHTML(r, i, tDesk, tMob) {
    ...
    else if(s.includes("PENDIENTE") || s.includes("SIN") || s.includes("DIAGNOSTICO") || s.includes("FALTA") || s.includes("AUTORIZAR") || s.includes("INGRESO")) { c = "row-pendiente"; badgeColor = "bg-orange-100 text-orange-700"; }
...
function renderPasosSeguimiento(d) {
    ...
    if(estado === "SIN INGRESAR A SISTEMA" || estado === "PENDIENTE" || estado === "") {
        // CORRECCIÓN VISUAL: ESTADO 'INGRESO', NO 'FALTA INSPECCION'
...
// ====================================================================
// CORRECCIÓN PRINCIPAL AQUÍ: ELIMINADO EL SALTO A "EN PROCESO"
// ====================================================================
function avanzarEstado(nuevoEstado, accion) {
    if(!confirm("¿Confirmar cambio?")) return;
    const d = datosProg[indiceActual];
    d.estado = nuevoEstado; // AHORA RESPETA EL "INGRESO" O "DIAGNOSTICO" QUE VIENE DEL BOTÓN
